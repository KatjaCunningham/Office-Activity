...
``` r
theoffice <- theoffice %>%
  mutate(text = tolower(text)) #makes all text lower case
...
office_df <- theoffice %>%
  ...
  left_join(halloween, by = "episode_name") %>%
  left_join(valentines, by = "episode_name") %>%
  left_join(christmas, by = "episode_name") %>%
  ...
  left_join(office_line, by = c("season", "episode", "episode_name"))
```
...
``` r
office_test_pred_old <- predict(office_fit_old, 
                                new_data = office_test) %>%
  bind_cols(office_test %>% select(imdb_rating, episode_name))
